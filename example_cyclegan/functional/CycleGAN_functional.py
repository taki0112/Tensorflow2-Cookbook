...
class CycleGAN() :
    ...
    def build_model(self):
        if self.phase == 'train' :
            ...
            self.source_generator = Generator(input_shape, self.ch, self.n_res, name='source_generator')
            self.target_generator = Generator(input_shape, self.ch, self.n_res, name='target_generator')
            self.source_discriminator = Discriminator(input_shape, self.ch, self.n_dis, self.sn, name='source_discriminator')
            self.target_discriminator = Discriminator(input_shape, self.ch, self.n_dis, self.sn, name='target_discriminator')
            ...
            self.target_generator.build_summary()
            self.target_discriminator.build_summary()
            ...
            """ Checkpoint """
            self.ckpt = tf.train.Checkpoint(source_generator=self.source_generator, target_generator=self.target_generator,
                                            source_discriminator=self.source_discriminator, target_discriminator=self.target_discriminator,
            ...
            self.source_generator.build_summary()
            self.source_discriminator.build_summary()
            self.target_generator.build_summary()
            self.target_discriminator.build_summary()
            ...
            print("Total Network parameter : ", format(params, ','))
    ...
    def g_train_step(self, real_a, real_b):
        with tf.GradientTape(persistent=True) as g_tape:
            ...
            self.g_adv_loss = self.adv_weight * (generator_loss(self.gan_type, fake_a_logit) + generator_loss(self.gan_type, fake_b_logit))
            self.g_cyc_loss = self.cyc_weight * (L1_loss(x_aba, real_a) + L1_loss(x_bab, real_b))
            self.g_identity_loss = self.identity_weight * (L1_loss(x_aa, real_a) + L1_loss(x_bb, real_b))

            regular_loss = regularization_loss(self.source_generator) + regularization_loss(self.target_generator)

            self.g_loss = self.g_adv_loss + self.g_cyc_loss + self.g_identity_loss + regular_loss

        g_train_variable = self.source_generator.trainable_variables + self.target_generator.trainable_variables
        g_gradient = g_tape.gradient(self.g_loss, g_train_variable)
    ...
    def d_train_step(self, real_a, real_b):
        with tf.GradientTape(persistent=True) as d_tape:
            ...
            self.d_adv_loss = self.adv_weight * (discriminator_loss(self.gan_type, real_a_logit, fake_a_logit) + discriminator_loss(self.gan_type, real_b_logit, fake_b_logit))

            regular_loss = regularization_loss(self.source_discriminator) + regularization_loss(self.target_discriminator)
            self.d_loss = self.d_adv_loss + regular_loss

        d_train_variable = self.source_discriminator.trainable_variables + self.target_discriminator.trainable_variables
        d_gradient = d_tape.gradient(self.d_loss, d_train_variable)
    ...
    def test(self):
        test_A_dataset = glob('./dataset/{}/{}/*.jpg'.format(self.dataset_name, 'testA')) + glob('./dataset/{}/{}/*.png'.format(self.dataset_name, 'testA'))
        ...
        ######################################################################################
        test_B_dataset = glob('./dataset/{}/{}/*.jpg'.format(self.dataset_name, 'testB')) + glob('./dataset/{}/{}/*.png'.format(self.dataset_name, 'testB'))
